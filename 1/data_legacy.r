...
data <- read.table("data_legacy.txt", header = TRUE)
...
# Этот коэффициент близок к нулю, что говорит о слабой корреляционной зависимости между этими переменными.  # nolint
# Таким образом, на основе анализа диаграммы рассеяния и коэффициента корреляции можно сделать вывод о том, что общей зависимости между переменными не наблюдается. # nolint
